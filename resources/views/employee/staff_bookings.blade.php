...
<div class="dash__block">
	...
	<form class="request" method="POST" action="/staff/bookings">
		...
		<div class="form-group">
			...
			<select name="employee_id" id="booking_employee" class="form-control request__input" onchange="showRedirect('.loading', '/staff/bookings/{{ $dateString }}/' + this.value)">
	...
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane active" id="ava">
			<div class="dash__block">
				...
				<div class="form-group">
				    ...
				    <select name="month_year" id="input_month_year" class="form-control request__input" onchange="location = '/staff/bookings/' + this.value + '{{ $employee ? '/' . $employee->id : null }}#available'">
		...
		<div role="tabpanel" class="tab-pane" id="book">
			...
			<div class="form-group">
				<label for="input_month_year">Month & Year <span class="request__validate">(Select to go to month)</span></label>
			    <select name="month_year" id="input_month_year" class="form-control request__input" onchange="location = '/staff/bookings/' + this.value + '#bookings'">
			        @foreach ($months as $month)
			            <option value="{{ $month->format('m-Y') }}" {{ $date->format('m-Y') == $month->format('m-Y') ? 'selected' : null }}>{{ $month->format('F Y') }}</option>
			        @endforeach
			    </select>
						...
						<tr>
							...
							<td class="table__date">
								@if ($booking->status === 'Pending' && !$booking->employee_id)
									<a href="/staff/bookings/{{ $booking->id }}/assign-doctor" class="btn btn-sm btn-info" style="margin-bottom:4px;">Assign Doctor</a>
									<form method="POST" action="/staff/bookings/{{ $booking->id }}/cancel" style="display:inline-block;margin-left:4px;">
										{{ csrf_field() }}
										<button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Cancel this appointment?')">Cancel</button>
									</form>
								@elseif ($booking->status === 'Pending')
									<form method="POST" action="/staff/bookings/{{ $booking->id }}/approve" style="display:inline-block;">
										{{ csrf_field() }}
										<button type="submit" class="btn btn-sm btn-success">Approve</button>
									</form>
									<a href="/staff/bookings/{{ $booking->id }}/assign-doctor" class="btn btn-sm btn-info" style="margin-left:4px;">Reassign</a>
									<form method="POST" action="/staff/bookings/{{ $booking->id }}/cancel" style="display:inline-block;margin-left:4px;">
										{{ csrf_field() }}
										<button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Cancel this appointment?')">Cancel</button>
									</form>
								@elseif ($booking->status === 'Approved')
									<form method="POST" action="/staff/bookings/{{ $booking->id }}/complete" style="display:inline-block;">
										{{ csrf_field() }}
